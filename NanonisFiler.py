...
class Nanofile:
    def __init__(self, filename, extension, mType = None, path=None):
        if path != None:
            ph.make_dir("%s"%path)
        ...
        if mType == "1D sweep":
            data = Lecture(filename, extension)[1:]
            self.nAcqChan = data.shape[0]-1
            self.NPoints = data.shape[1]
            self.xData = data[0]
            self.xStart = self.xData[0]
            self.xStop = self.xData[-1]
            self.data = data[1:]
            self.acqInd = 0
            self.xName = ''
            self.xUnit = ''
    ...
    def plot(self, acqIndex=0,  color = cm.bone, colorbar = False, *args, **kwargs):
        if self.measType == "2D sweep":
            plt.clf()
            ...
            if colorbar == True:
                plt.colorbar()
        elif self.measType == "1D sweep":
            plt.clf()
            plt.plot(self.xData, self.data[self.acqInd], *args, **kwargs)
            plt.ylabel("%(yname)s (%(yunit)s)" %{"yname":self.yName, "yunit":self.yUnit})
            plt.xlabel("%(xname)s (%(xunit)s)" %{"xname":self.xName, "xunit":self.xUnit})
            plt.title("%(filen)s -%(dset)s" %{"filen":self.filename, "dset":"original"})
        else:
            raise NotImplementedError
